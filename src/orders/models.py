...
def add_key_to_cart_obj(sender, instance, *args, **kwargs):
    if not instance.key:
        instance.key = unique_key_generator(instance, size=12)


pre_save.connect(add_key_to_cart_obj, sender=Cart)


def add_key_to_discount_obj(sender, instance, *args, **kwargs):
    if not instance.key:
        instance.key = unique_key_generator(instance, size=randint(6, 10))


pre_save.connect(add_key_to_discount_obj, sender=Discount)


def add_order_id_to_order_obj(sender, instance, *args, **kwargs):
...
pre_save.connect(add_order_id_to_order_obj, sender=Order)
